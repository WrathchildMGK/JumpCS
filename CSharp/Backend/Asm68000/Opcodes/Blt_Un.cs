...
public class Blt_Un : IOpcodeTranslation
{
    public void Translate(object? operand, IBackendSupport support)
    {
        ...
        else if (support.Stack.StackDepth >= 2)
        {
            ...
            support.AsmWriter.WriteLine($"    BCS {label}  ; Branch if < (unsigned - Carry Set)");
        }
        else { support.AsmWriter.WriteLine($"    ; WARNING: Blt_Un with insufficient stack depth ({support.Stack.StackDepth})"); }
    }
}