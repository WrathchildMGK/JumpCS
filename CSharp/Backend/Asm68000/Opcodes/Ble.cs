...
public class Ble : IOpcodeTranslation
{
    public void Translate(object? operand, IBackendSupport support)
    {
        ...
        else if (support.Stack.StackDepth >= 2)
        {
            ...
            support.AsmWriter.WriteLine($"    BLE {label}  ; Branch if <= (signed)");
        }
        else { support.AsmWriter.WriteLine($"    ; WARNING: Ble with insufficient stack depth ({support.Stack.StackDepth})"); }
    }
}