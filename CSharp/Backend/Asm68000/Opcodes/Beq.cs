...
public class Beq : IOpcodeTranslation
{
    public void Translate(object? operand, IBackendSupport support)
    {
        if (support.Stack.StackDepth >= 4 && support.Stack.IsTopDoubleWord)
        {
            ...
            support.AsmWriter.WriteLine($"    BNE {label}  ; Branch if equal (double)");
        ...
        else if (support.Stack.StackDepth >= 2)
        {
            ...
            support.AsmWriter.WriteLine($"    BEQ {label}  ; Branch if equal (signed)");
        }
        else { support.AsmWriter.WriteLine($"    ; WARNING: Beq with insufficient stack depth ({support.Stack.StackDepth})"); }
    }
}