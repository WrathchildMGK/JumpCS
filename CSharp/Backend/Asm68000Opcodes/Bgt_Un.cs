...
public class Bgt_Un : IOpcodeTranslation
{
    public void Translate(object? operand, Asm68000Support support)
    {
        ...
        else if (support.Stack.StackDepth >= 2)
        {
            ...
            support.AsmWriter.WriteLine($"    BHI {label}  ; Branch if > (unsigned)");
        }
        else { support.AsmWriter.WriteLine($"    ; WARNING: Bgt_Un with insufficient stack depth ({support.Stack.StackDepth})"); }
    }
}